...
from .config import settings, ModelTask, ProviderType
from .providers.base import LlmProvider, ChatMessage, ChatResponse, EmbeddingResponse
from .providers.litellm_provider import LiteLlmProvider
from .resilience import CircuitBreaker
from .pricing import estimate_cost, find_cheapest_model
    ...
    async def route_with_fallback(
        ...
        **kwargs
        ...
        for provider_id, model_id, priority in candidates:
            ...
            if preferred_model:
                reason = f"Matched preferred model"
            ...
            for cache_key, model_info in self.model_cache.items():
                if model_info.model_id == preferred_model:
                    provider_config = next(
                        (p for p in settings.providers if p.id == model_info.provider_id),
                        None
                    )
                    if provider_config:
                        candidates.append((
    ...
    def get_provider_health(self) -> Dict[str, Dict]:
        ...
        for provider_id, provider in self.providers.items():
            ...
            health[provider_id] = {
                ...
                "models_count": len([
                    m for m in self.model_cache.values()
